		...
		switch ($action) {
			case 'acc':
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					$tgl_pic2=test($_POST['tgl_pic2']);
					if ($a=mysqli_query($conn, "UPDATE upp 	SET 	tgl_pic2 = '$tgl_pic2'
												WHERE 	no_upp = '$id'
												")){
						$a=mysqli_query($conn, "SELECT * FROM upp WHERE no_upp='$id' AND  status='approval'");
						$c=mysqli_fetch_array($a);
						if ($c['tgl_pic1']!='0000-00-00') {
							$a=mysqli_query($conn, "UPDATE upp SET status = 'approved', tgl_approved = '$tgl_pic2 ".date("H:i:s")."' WHERE no_upp = '$id'");
						}
				...
				$a=mysqli_query($conn, "SELECT * FROM upp WHERE no_upp='$id' AND status='approval' AND tgl_pic2='0000/00/00'");
				$c=mysqli_fetch_array($a);
				echo "
					<div id='popup' class='popup'>
						<a href='main?index=upp&step=approval2&id=$id'>
						...
						<div class='form_process'>
							<form action='main?index=upp&step=approval2&action=acc&id=$id' method='post'>
								...
								<input class='input_main' type='date' name='tgl_pic2' value='$hariini' required style='background-color:#e7e7e7;width:100%;' readonly><br>
			...
			case 'reject':
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					$alasan='Pic 2 : '.test($_POST['alasan']);
				...
				echo "
					<div id='popup' class='popup'>
						...
						<div class='form_process'>
							<form action='main?index=upp&step=approval2&action=reject&id=$id' method='post'>
								<a class='j_input_main'>No. UPP</a><br>
								<input class='input_main' type='text' name='no_upp' value='$c[no_upp]' required style='background-color:#e7e7e7;width:100%;' readonly><br>
				...
				break;
			case 'autosent':
				$a=mysqli_query($conn, "UPDATE upp SET tgl_kirim2 = '$hariini' WHERE no_upp ='$id'");
				$a=mysqli_query($conn, "SELECT * FROM upp 
					inner join divisi_prosedur on upp.no_divisi_prosedur = divisi_prosedur.no_divisi_prosedur 
					inner join master_prosedur on upp.no_master_prosedur = master_prosedur.no_master_prosedur 
					inner join jenis_prosedur on upp.no_jenis_prosedur = jenis_prosedur.no_jenis_prosedur
				...
				if (Pear::isError($mail)) {
					...
					echo "
						<div id='popup' class='popup'>
							...
							<div class='popup_upp'>
								<a href='main?index=upp&step=approval2&id=$id' class='close-button' title='close'>X</a>
								UPP ONLINE<br>
			...
			case 'batal':
				...
				echo "
					<div id='popup' class='popup'>
						...
						<div class='form_process'>
							<form action='main?index=upp&step=approval2&action=batal&id=$id' method='post'>
								<a class='j_input_main'>No. UPP</a><br>
				...
				echo "
					<div id='popup' class='popup'>
						...
						<div class='form_process' style='overflow:auto;width:460px;height:470px;'>
							<form action='main?index=upp&step=approval2&action=edit&id=$id#popup' method='post'  enctype='multipart/form-data'>
								<div class='form_main'>
									";
										if (isset($alert)) {
									...
									<select class='input_main' name='jenis' id='jenis_ik' onchange="check()" required>
										<option value=''></option>
										<?php
									...
									<input class='file_main' type='file' id='file_fmea' name='file_fmea' onchange="valid(this)"><br id="file_fmea">
...
?>
<div class='judul_main' style='position: fixed;'>Approval PIC 2 Usulan Perubahan Prosedur</div>
<div class='form_main' style='margin-top: 46px;'>
	<?php
	echo"<form  name='search' style='margin-bottom:0px;' action='main?index=upp&step=approval2' method='post' enctype='multipart/form-data'>
		...
		if (isset($_GET['id'])) {
			$a=mysqli_query($conn, "SELECT * FROM upp 
				...
				inner join jenis_prosedur on upp.no_jenis_prosedur = jenis_prosedur.no_jenis_prosedur
				inner join jenis_ik	on upp.jenis_ik = jenis_ik.kode_ik
				WHERE no_upp='$id' AND status='approval' AND tgl_pic2='0000/00/00'");
			echo "
				<div class='alert_adm alert2'>id : $id<a href='main?index=upp&step=approval2' style='font-family:arial;color:000;float:right;'>X</a><div class='cb'></div></div>
			";
		}
		else if (isset($_SESSION['username']) and $_SESSION['level'] == "qa")
		{
			$a=mysqli_query($conn, "SELECT * FROM upp 
				inner join divisi_prosedur on upp.no_divisi_prosedur = divisi_prosedur.no_divisi_prosedur 
				inner join master_prosedur on upp.no_master_prosedur = master_prosedur.no_master_prosedur 
				inner join jenis_prosedur on upp.no_jenis_prosedur = jenis_prosedur.no_jenis_prosedur 
				inner join jenis_ik	on upp.jenis_ik = jenis_ik.kode_ik
				WHERE status='approval' AND tgl_pic2='0000/00/00' and pic2 = '".$_SESSION['email']."' ORDER BY tgl_kirim desc");
		}
		else{
			$a=mysqli_query($conn, "SELECT * FROM upp 
				inner join divisi_prosedur on upp.no_divisi_prosedur = divisi_prosedur.no_divisi_prosedur 
				...
				inner join jenis_prosedur on upp.no_jenis_prosedur = jenis_prosedur.no_jenis_prosedur 
				inner join jenis_ik	on upp.jenis_ik = jenis_ik.kode_ik
				WHERE status='approval' AND tgl_pic2='0000/00/00' ORDER BY tgl_kirim desc");
		}
		if(isset($_POST['kw']))
			{
				$kw = $_POST['kw'];
			$a=mysqli_query($conn, "SELECT * FROM upp 
				...
				inner join jenis_prosedur on upp.no_jenis_prosedur = jenis_prosedur.no_jenis_prosedur 
				inner join jenis_ik	on upp.jenis_ik = jenis_ik.kode_ik
				WHERE status='approval' AND tgl_pic2='0000/00/00' 
...
<?php
	...
	while ($c=mysqli_fetch_array($a)) {
		if (isset($_SESSION['username'])) {
			...
			$finish = new DateTime($c['tgl_kirim2']);
			$leadtime=$start->diff($finish);
			$leadtime=$leadtime->days;
			if ($leadtime>0) {
				//header('location:main?index=upp&step=approval2&action=autosent&id='.$c['no_upp'].'#popup');
			}
		}
			...
			echo "
				<tr class='main_table odd'>
					<td style='padding:10px;'>
						<a style='padding-right:5px;color: blue;' href='main?index=upp&step=approval2&action=acc&id=$c[no_upp]#popup'>
							<img class='material-icons tiny' style='width:12px; height:9px; margin: 2px 0px 0px 3px;' src='img/done.png'> approve
						</a>
					</td>
					<td style='padding:10px;'>
						<a style='padding-right:5px;color: blue;' href='main?index=upp&step=approval2&action=reject&id=$c[no_upp]#popup'>
					...
					";
						if (isset($_SESSION['username'])) {
							echo "
								<td style='padding:10px;'>
									<a style='padding-right:5px;color: blue;' href='main?index=upp&step=approval2&action=sent&id=$c[no_upp]#popup'>
										<img class='material-icons tiny' style='width:15px; height:9px; margin: 2px 0px 0px 3px;' src='img/mail.png'> mail
									</a>
								</td>
								<td style='padding:10px;'>
									<a style='padding-right:5px;color: blue;' href='main?index=upp&step=approval2&action=batal&id=$c[no_upp]#popup'>
										<img class='material-icons tiny' style='width:10px; height:10px; margin: 2px 0px 0px 3px;' src='img/warning.png'> batal
									</a>
								</td>
								<td style='padding:10px;'>
									<a style='padding-right:5px;color: blue;' href='main?index=upp&step=approval2&action=edit&id=$c[no_upp]#popup'>
